...
describe('EJS Visual Feedback with Example File', () => {
  ...
  describe('Visual feedback with form-modal.ejs', () => {
    it('should provide highlights for _viewsPath variable', () => {
      ...
      for (let i = 1; i < lines.length; i++) { // Start from line 1 to skip definition
        const line = lines[i];
      ...
      if (testPosition) {
        ...
        const result = provider.provideDocumentHighlights(document, testPosition, token);
        ...
        if (Array.isArray(result)) {
          ...
          const writeHighlights = result.filter(h => h.kind === vscode.DocumentHighlightKind.Write);
          const readHighlights = result.filter(h => h.kind === vscode.DocumentHighlightKind.Read);
    ...
    it('should handle complex variable expressions', () => {
      ...
      if (testPosition) {
        ...
        const token = { isCancellationRequested: false };
        const highlights = provider.provideDocumentHighlights(document, testPosition, token);
    ...
    it('should return false for undefined variables', () => {
      ...
      const testPosition = new vscode.Position(lines.length - 1, variableIndex + 1);
    ...
    it('should handle variables with similar names correctly', () => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (!socialChannelNamePosition) {
          ...
          if (nameMatch) {
            socialChannelNamePosition = new vscode.Position(i, nameMatch.index! + 1);
          }
        }
        ...
        if (!socialChannelPosition) {
          ...
          if (channelMatch) {
            socialChannelPosition = new vscode.Position(i, channelMatch.index! + 1);
          }
        }
      ...
      if (socialChannelNamePosition) {
        const result1 = provider.isOverDefinedVariable(document, socialChannelNamePosition);
        expect(result1).toBe(true);
      }
      
      if (socialChannelPosition) {
        const result2 = provider.isOverDefinedVariable(document, socialChannelPosition);
    ...
    it('should perform well with the large example file', () => {
      ...
      let testPosition: vscode.Position | null = null;
      for (let i = middleLine; i < Math.min(middleLine + 10, lines.length); i++) {
        const line = lines[i];
