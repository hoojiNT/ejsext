...
vi.mock("vscode", () => ({
  ...
  Location: vi.fn().mockImplementation((uri: any, position: any) => ({
    uri,
    range: position,
  })),
  Hover: vi
...
describe("EJSHoverProvider Integration", async () => {
  ...
  beforeEach(() => {
    ...
    const cache = new DocumentCacheManager();
    hoverProvider = new EJSHoverProvider(parser, symbolAnalyzer, cache);
  });

  it("should provide hover information for _viewsPath variable in example EJS file", async () => {
...
<div class="modal modal-lg fade" id="modal-salestream-channel-form">
  ...
  it("should provide hover information for _editing variable", async () => {
    const ejsContent = `<% const _viewsPath = '../../../..'; %>
<% const _editing = (typeof editing !== 'undefined' && editing) ? editing : {}; %>
<% const _socialChannel = _editing?.socialChannel || {}; %>
...
<div>
  ...
  it("should return null for variables not inside EJS blocks", async () => {
    const ejsContent = `<% const _viewsPath = '../../../..'; %>
<div class="modal">
